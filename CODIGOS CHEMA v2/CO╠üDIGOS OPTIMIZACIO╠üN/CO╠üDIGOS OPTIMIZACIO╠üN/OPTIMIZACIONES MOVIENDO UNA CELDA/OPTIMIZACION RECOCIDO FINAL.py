...
def valid_assignment_original(assignment, original_assignment, max_changes=1):
    """
    Verifica que, comparando con la asignación original, cada sector
    solo haya perdido (o cambiado) como máximo max_changes celdas.
    """
    changes = {}
    for cell in assignment:
...
# =============================================================================
# 5. DEFINICIÓN DE LA FUNCIÓN OBJETIVO CON REPARTO DE COMPLEJIDAD ENTRE SECTORES
# =============================================================================
...
max_change = 1  # Cada sector puede cambiar como máximo 1 celda

# =============================================================================
# IMPLEMENTACIÓN DEL ALGORITMO DE SIMULATED ANNEALING CON LA NUEVA RESTRICCIÓN
# =============================================================================
def simulated_annealing(assignment, original_assignment, G, max_change, initial_temp=1000, cooling_rate=0.99, max_iterations=1000):
    current_assignment = assignment.copy()
    current_obj_value = improved_objective(current_assignment)
    temp = initial_temp

    for iteration in range(max_iterations):
        border_cells = get_border_cells(current_assignment)
        if not border_cells:
            continue
        random_cell = random.choice(border_cells)
        current_sec = current_assignment[random_cell]
        candidate_sectors = {current_assignment[neighbor] for neighbor in G.neighbors(random_cell)}
        candidate_sectors.discard(current_sec)
        if not candidate_sectors:
            continue
        new_sec = random.choice(list(candidate_sectors))
        if check_move_connectivity(current_assignment, current_sec, new_sec, random_cell):
            new_assignment = current_assignment.copy()
            new_assignment[random_cell] = new_sec
            # Verificamos la restricción: cada sector según la asignación original solo puede cambiar 1 celda.
            if not valid_assignment_original(new_assignment, original_assignment, max_changes=max_change):
                continue
            new_obj_value = improved_objective(new_assignment)
            # Aceptamos la nueva solución si es mejor, o con una probabilidad si es peor
            if new_obj_value < current_obj_value or random.random() < math.exp((current_obj_value - new_obj_value) / temp):
                current_assignment = new_assignment
                current_obj_value = new_obj_value
        temp *= cooling_rate
    return current_assignment, current_obj_value

# =============================================================================
# EJECUCIÓN DE LA OPTIMIZACIÓN CON SIMULATED ANNEALING
# =============================================================================
optimized_assignment, optimized_obj_value = simulated_annealing(assignment, original_assignment, G, max_change)

# Actualizamos el DataFrame con la asignación optimizada
df_cells['Optimized_Sector'] = df_cells['Cell_Name'].map(optimized_assignment)
