...
max_change = 20  # Cada sector puede perder como máximo x celdas


# =============================================================================
# IMPLEMENTACIÓN DEL ALGORITMO GENÉTICO CON RESTRICCIÓN EN EL CRUCE
# =============================================================================
def crossover(parent1, parent2, original_assignment, max_changes):
    """
    Realiza un cruce simple entre dos padres y verifica que los hijos resultantes cumplan
    la restricción de que cada sector (según la asignación original) solo cambie un máximo de max_changes celdas.
    Si un hijo viola la restricción, se devuelve el padre correspondiente.
    """
    ...
    if not valid_assignment_original(child1, original_assignment, max_changes):
        child1 = parent1.copy()
    if not valid_assignment_original(child2, original_assignment, max_changes):
        child2 = parent2.copy()
    return child1, child2

def mutate(assignment, original_assignment, max_changes=30):
    border_cells = get_border_cells(assignment)
    if not border_cells:
        return assignment
    random_cell = random.choice(border_cells)
    current_sec = assignment[random_cell]
    candidate_sectors = {assignment[neighbor] for neighbor in G.neighbors(random_cell)}
    candidate_sectors.discard(current_sec)
    if not candidate_sectors:
        return assignment
    new_sec = random.choice(list(candidate_sectors))
    if check_move_connectivity(assignment, current_sec, new_sec, random_cell):
        new_assignment = assignment.copy()
        new_assignment[random_cell] = new_sec
        if valid_assignment_original(new_assignment, original_assignment, max_changes):
            return new_assignment
    return assignment

def genetic_algorithm(assignment, original_assignment, max_changes, population_size=20, generations=10):

    population = []
    for _ in range(population_size):
        sol = mutate(assignment.copy(), original_assignment, max_changes)
    ...
    for gen in range(generations):
        population.sort(key=lambda x: improved_objective(x))
        if improved_objective(population[0]) < best_obj_value:
            best_solution = population[0]
            best_obj_value = improved_objective(population[0])
        next_generation = population[:population_size // 2]
        while len(next_generation) < population_size:
            parent1, parent2 = random.sample(next_generation, 2)
            child1, child2 = crossover(parent1, parent2, original_assignment, max_changes)
            next_generation.append(mutate(child1, original_assignment, max_changes))
            next_generation.append(mutate(child2, original_assignment, max_changes))
        population = next_generation
    return best_solution, best_obj_value

# Llamamos al algoritmo genético
best_assignment, best_obj_value = genetic_algorithm(assignment, original_assignment, max_change)
