...
def check_move_connectivity(assignment, old_sec, new_sec, cell):
    ...
    # Ahora pasamos 'G' como argumento
...
# SIMULATED ANNEALING
# =============================================================================
def simulated_annealing(assignment, G, initial_temp=1000, cooling_rate=0.99, max_iterations=1000):
    current_assignment = assignment.copy()
    current_obj_value = improved_objective(current_assignment)
    
    # Empezamos con una temperatura alta
    temp = initial_temp
    
    for iteration in range(max_iterations):
        # Generamos una solución vecina (movemos una celda a un sector vecino)
        ...
        # Seleccionamos los sectores vecinos
        candidate_sectors = {assignment[neighbor] for neighbor in G.neighbors(random_cell)}
        candidate_sectors.discard(current_sec)  # No podemos mover la celda a su propio sector
        
        # Verificamos si hay sectores disponibles
        if not candidate_sectors:
            continue  # Si no hay sectores disponibles, pasamos a la siguiente celda
        
        # Elegimos aleatoriamente un sector vecino
        new_sec = random.choice(list(candidate_sectors))
        
        # Verificamos si el movimiento es válido (mantiene la conectividad de ambos sectores)
...
# =============================================================================
# EJECUCIÓN DE LA OPTIMIZACIÓN CON SIMULATED ANNEALING Y MEDICIÓN DEL TIEMPO
# =============================================================================

optimized_assignment, optimized_obj_value = simulated_annealing(assignment, G)
