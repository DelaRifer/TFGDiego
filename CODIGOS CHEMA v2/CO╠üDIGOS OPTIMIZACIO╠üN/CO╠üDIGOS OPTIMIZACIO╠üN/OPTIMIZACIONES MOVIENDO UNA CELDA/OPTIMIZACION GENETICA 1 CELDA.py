...
# # =============================================================================
#  OPTIMIZACION GENETICA
# =============================================================================

def crossover(parent1, parent2):
    # Realiza un cruce simple entre dos padres (asignación de celdas)
...
def mutate(assignment):
    # Realiza una mutación aleatoria moviendo una celda a un sector vecino
    ...
    return assignment  # No cambiamos si no es válido

def genetic_algorithm(assignment, population_size=50, generations=100):
    # Inicializamos la población con soluciones aleatorias
    population = [assignment.copy() for _ in range(population_size)]
    best_solution = assignment
    best_obj_value = improved_objective(assignment)
    
    for gen in range(generations):
        # Evaluamos la función objetivo para cada individuo de la población
        population.sort(key=lambda x: improved_objective(x))  # Ordenamos por la mejor solución
        if improved_objective(population[0]) < best_obj_value:
            best_solution = population[0]
            best_obj_value = improved_objective(population[0])
        
        # Seleccionamos los mejores para la reproducción
        next_generation = population[:population_size // 2]
        
        # Realizamos cruces y mutaciones para generar la siguiente generación
        ...
        population = next_generation  # Actualizamos la población
    
    return best_solution, best_obj_value

# Llamada a la función de optimización genética
best_assignment, best_obj_value = genetic_algorithm(assignment)

# Asignamos la columna 'Optimized_Sector' en df_cells con la asignación optimizada
df_cells['Optimized_Sector'] = df_cells['Cell_Name'].map(best_assignment)

# Imprimir la mejor asignación y el valor final de la función objetivo
print("Mejor asignación de sectores:", best_assignment)
print("Valor final de la función objetivo:", best_obj_value)
