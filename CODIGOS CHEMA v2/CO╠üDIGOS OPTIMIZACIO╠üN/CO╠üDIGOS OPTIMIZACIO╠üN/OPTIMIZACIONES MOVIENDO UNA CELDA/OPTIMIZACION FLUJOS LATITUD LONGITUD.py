...
Complejidad_celdas_ = pd.read_pickle(PATH_COMPLEJIDAD_CELDA + 'Complejidad_por_hora_2022-06-01_07-08.pkl')
...
def check_flow_orientation(cell, current_sec, candidate_sec, df_flujos, gdf_sectors):
    """
    Comprueba que la orientación predominante de los flujos asociados a 'cell'
    coincide con la dirección del movimiento propuesto (horizontal o vertical).

    Parámetros:
      - cell: identificador de la celda (ej. 'Cell_Name').
      - current_sec: sector actual de la celda.
      - candidate_sec: sector candidato hacia el que se propone mover la celda.
      - df_flujos: DataFrame con los flujos, que debe incluir las columnas
                   'lon_cell_in', 'lat_cell_in', 'lon_cell_out', 'lat_cell_out'
                   y 'Cell_Name' que relaciona cada flujo con la celda de paso.
      - gdf_sectors: GeoDataFrame con la información de los sectores, que debe tener
                     una columna identificadora (por ejemplo 'SECTOR_ID') y su geometría.

    Retorna:
      - True si, según los flujos asociados, la mayoría indica una orientación
        coherente con el movimiento (horizontal o vertical); en caso contrario, False.
      - Si no se encuentran flujos asociados a la celda, se retorna True para no 
        bloquear el movimiento por falta de información.
    """
    # Usamos representative_point en lugar de centroid para garantizar que el punto
    # se encuentre dentro de la geometría del sector.
    point_current = gdf_sectors[gdf_sectors['SECTOR_ID'] == current_sec].geometry.representative_point().iloc[0]
    point_candidate = gdf_sectors[gdf_sectors['SECTOR_ID'] == candidate_sec].geometry.representative_point().iloc[0]
    dx_move = point_candidate.x - point_current.x
    dy_move = point_candidate.y - point_current.y

    # Definir la dirección del movimiento según cuál diferencia sea mayor
    if abs(dx_move) >= abs(dy_move):
        move_dir = 'horizontal'
    else:
        move_dir = 'vertical'

    # Filtrar los flujos asociados a la celda
    cell_flows = df_flujos[df_flujos['Cell_Name'] == cell]
    if cell_flows.empty:
        return True  # Si no hay flujos, no se bloquea el movimiento

    horizontal_count = 0
    vertical_count = 0

    for idx, flow in cell_flows.iterrows():
        dx_flow = abs(flow['lon_cell_out'] - flow['lon_cell_in'])
        dy_flow = abs(flow['lat_cell_out'] - flow['lat_cell_in'])
        if dx_flow > dy_flow:
            horizontal_count += 1
        elif dy_flow > dx_flow:
            vertical_count += 1
...
# -----------------------------------------------------------------------------
# CARGAR EL DATASET COMPLETO DE FLUJOS DESDE EL ARCHIVO PKL
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# CREAR LA GEOMETRÍA DE CADA FLUJO
# -----------------------------------------------------------------------------
...
# =============================================================================
# 7. OPTIMIZACIÓN ITERATIVA CON REPARTO DE COMPLEJIDAD ENTRE SECTORES - n CORRIDAS
# =============================================================================
n_runs = 10
    ...
    while improved:
        ...
        for cell in border_cells:
            ...
            for cand in candidate_sectors:
                ...
                # NUEVA CONDICIÓN: comprobar la orientación de los flujos
                if not check_flow_orientation(cell, current_sec, cand, df_flujos, gdf_sectors):
                    continue
...
# -----------------------------------------------------------------------------
# 5. REPRESENTAR EL MALLADO Y SUPERPONER LOS FLUJOS
# -----------------------------------------------------------------------------
fig, ax_cells = plt.subplots()
...
# -----------------------------------------------------------------------------
# Pintar los flujos de TODAS las celdas (por ejemplo, en azul)
# -----------------------------------------------------------------------------
first_all = True
for _, row in gdf_flujos.iterrows():
    ...
    if line is not None:
        ...
        if first_all:
            ax_cells.plot(x_line, y_line, color='blue', linewidth=1, alpha=0.5, label='Flujos totales')
            first_all = False
        else:
            ax_cells.plot(x_line, y_line, color='blue', linewidth=1, alpha=0.5)

# -----------------------------------------------------------------------------
# Pintar los flujos de las celdas CAMBIADAS (en rojo) para resaltarlos
# -----------------------------------------------------------------------------
primer_flujo = True
...
ax_cells.set_aspect('equal')
ax_cells.set_title("MALLADO DEL ESPACIO AÉREO CON FLUJOS")
ax_cells.set_xlabel('LONGITUD [º]')
ax_cells.set_ylabel('LATITUD [º]')
plt.legend(loc='upper center', bbox_to_anchor=(0.5, -0.25), ncol=3, fontsize='small')

# Guardar la figura
nombre_figura = PATH_COMPLEJIDAD_OPT + 'Mallado del espacio aéreo de estudio con todos los flujos y flujos en celdas cambiadas.png'
