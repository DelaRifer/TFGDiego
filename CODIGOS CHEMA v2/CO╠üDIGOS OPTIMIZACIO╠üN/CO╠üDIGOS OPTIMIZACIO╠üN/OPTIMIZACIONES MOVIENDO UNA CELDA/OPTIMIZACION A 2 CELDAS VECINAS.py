...
def remove_islands(assignment, original_assignment, max_change=6):
    """
    Reasigna celdas que estén completamente rodeadas por un único sector,
    siempre y cuando no rompa la conectividad ni exceda los cambios por sector.
    """
    changed = True
    while changed:
        changed = False
        # Hacemos una copia de la lista de celdas para no modificar sobre la marcha
        all_cells = list(assignment.keys())
        
        for cell in all_cells:
            current_sec = assignment[cell]
            neighbors = list(G.neighbors(cell))
            if not neighbors:
                continue  # Celdas sin vecinos (raro) se ignoran
            
            # Sectores de los vecinos
            neighbor_secs = {assignment[nbr] for nbr in neighbors}
            
            # Si todos los vecinos están en un único sector, candidate_sec será ese sector
            if len(neighbor_secs) == 1:
                candidate_sec = neighbor_secs.pop()
                
                # Solo si la celda está en un sector distinto
                if candidate_sec != current_sec:
                    # Verificamos que la celda realmente esté "rodeada" de candidate_sec
                    # y que no sea un simple borde con 2 sectores, etc.
                    # (En este caso, ya vimos que neighbor_secs tiene un único sector).
                    
                    # Chequeamos conectividad
                    if check_move_connectivity(assignment, current_sec, candidate_sec, cell):
                        
                        # Verificamos que no exceda los límites de celdas que puede perder/ganar un sector
                        new_assignment = assignment.copy()
                        new_assignment[cell] = candidate_sec
                        
                        changes = changed_cells_count(new_assignment, original_assignment)
                        if any(count > max_change for count in changes.values()):
                            continue  # Saltamos este cambio, excede el máximo
                        
                        # Si pasa todas las pruebas, forzamos el cambio
                        assignment[cell] = candidate_sec
                        changed = True
    return assignment
# =============================================================================
...
def shared_edge_penalty(assignment, min_edges=2, penalty_value=1000):
    """
    Para cada celda que tenga menos de `min_edges` vecinos del mismo sector,
    acumula un valor de penalización (por ejemplo, penalty_value por cada celda que incumpla).
    """
    penalty = 0
    for cell in assignment:
        same_neighbors = sum(1 for neighbor in G.neighbors(cell) if assignment[neighbor] == assignment[cell])
        if same_neighbors < min_edges:
...
def improved_objective(assignment):
    """
    Función objetivo que minimiza la varianza de la complejidad total entre sectores,
    penalizando además la diferencia extrema de complejidad y sumando una penalización 
    si alguna celda no cuenta con al menos 2 vecinos del mismo sector.
    """
    # 1. Complejidad total por sector
    comp_by_sector = {}
    for cell, sec in assignment.items():
        comp = df_cells.loc[df_cells['Cell_Name'] == cell, 'Complexity'].values[0]
        comp_by_sector.setdefault(sec, 0)
    ...
    # 2. Calcular la varianza muestral de la complejidad
    variance = 0
    if number_sectors > 1:
        variance = sum((v - overall_avg) ** 2 for v in values) / (number_sectors - 1)

    # 3. Penalización por diferencia extrema de complejidad
    complexity_diff_penalty = max(values) - min(values)

    # 4. Penalización por la ausencia de al menos 2 vecinos en el mismo sector
    penalty = shared_edge_penalty(assignment, min_edges=2, penalty_value=1000)

    # 5. Función objetivo combinada
    objective_value = variance + complexity_diff_penalty + penalty
    return objective_value
...
# =============================================================================
# Función auxiliar para identificar si la celda está en la frontera exterior del ACC
# =============================================================================
def is_outer_cell(cell):
    """
    Retorna True si la celda con nombre 'cell' tiene su polígono tocando
    (o intersectando) el límite del ACC.
    """
    # Se asume que df_cells contiene la columna 'Polygon' asociada a cada 'Cell_Name'
    poly = df_cells.loc[df_cells['Cell_Name'] == cell, 'Polygon'].values[0]
    # Se puede usar touches o intersects con el borde del ACC. Aquí usamos intersects.
    return poly.intersects(poligono_ACC.boundary)
...
for run in range(1, n_runs + 1):
    ...
    while improved:
        ...
        # Reordenamos las celdas de borde, priorizando las que están en la frontera exterior del ACC.
        # Las celdas que tocan el ACC se ordenan con prioridad (valor 0) y el resto con prioridad 1.
        border_cells = sorted(border_cells, key=lambda cell: 0 if is_outer_cell(cell) else 1)
        # Puedes combinar el sort con un shuffle en las que estén en la misma categoría si lo deseas.
        ...
        # Lógica de mover celdas de borde
        for cell in border_cells:
            ...
            for cand in candidate_sectors:
                ...
                if new_obj < current_obj:
                    ...
                    improved = True
                    current_obj = new_obj
                    break   # Salir del loop de candidatos una vez que se ha hecho un cambio
            if improved:
                break   # Salir del loop de celdas de borde tras mejorar

        # (Opcional) Puedes incorporar también la eliminación de islas o celdas minoritarias aquí,
        # llamando a la función remove_islands, tal como se mostró en el ejemplo anterior.
        assignment_run_before = assignment_run.copy()
        assignment_run = remove_islands(assignment_run, original_assignment, max_change=max_change)
        if assignment_run != assignment_run_before:
            improved = True
            current_obj = improved_objective(assignment_run)
            print(f"   -> Eliminando islas. Nuevo objetivo = {current_obj:.2f}")
