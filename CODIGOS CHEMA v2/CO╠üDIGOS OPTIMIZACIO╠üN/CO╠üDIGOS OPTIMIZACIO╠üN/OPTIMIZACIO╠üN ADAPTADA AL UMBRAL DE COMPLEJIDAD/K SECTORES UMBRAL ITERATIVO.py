...
# =============================================================================
# INCORPORAR LA COMPLEJIDAD DE LAS CELDAS
# =============================================================================
...
# =============================================================================
# CONSTRUIR EL GRAFO DE VECINOS BASADO EN LA CONTIGÜIDAD DE CELDAS (POR ARISTA)
# =============================================================================
...
##############################################
def split_to_k(assignment, cell_complexity, G, target_k):
...
def optimize_pairs(original_assignment, df_cells, G, max_change=40, n_runs=5):
    ...
    # -----------------------------------------------------------------------------
    # Se asume que en df_flujos existen las columnas:
    ...
    # -----------------------------------------------------------------------------
    # Se asume que df_cells ya existe y contiene las columnas "Sector", "Optimized_Sector" y "Cell_Name"
    ...
    # -----------------------------------------------------------------------------
    # Se asume que en df_flujos existe la columna "Cell_Name" que indica la celda de paso de cada flujo
...
# Asignación inicial y complejidad de celdas
...
# Umbral deseado de complejidad sectorial
threshold = float(input("Introduce el umbral de complejidad sectorial: "))

# Empezamos desde la asignación inicial
assignment = initial_assignment.copy()

def compute_sector_complexities(assignment, cell_complexity):
    """
    Devuelve un dict {sector: suma_de_complexities_de_celdas}
    """
    sector_comp = {}
    for cell, sec in assignment.items():
        sector_comp.setdefault(sec, 0)
        sector_comp[sec] += cell_complexity[cell]
    return sector_comp
...
while True:
    current_sectors = len(set(assignment.values()))
    target_k = current_sectors + 1
    print(f"\n=== Iteración {i}: creando sector #{target_k} ===")

    # 1) Crear un nuevo sector (split) partiendo de la asignación actual
    assignment = split_to_k(assignment, cell_complexity, G, target_k)

    # 2) Optimizar pares
    assignment = optimize_pairs(assignment, df_cells, G, max_change=40, n_runs=5)

    # Guardamos la asignación de esta iteración en el DataFrame
    df_cells[f'Sector_run_{i}'] = df_cells['Cell_Name'].map(assignment)

    # 3) Computar complejidad de cada sector
    sector_comp = compute_sector_complexities(assignment, cell_complexity)
    print("Complejidades sectoriales actuales:")
    for sec, comp in sector_comp.items():
        print(f"  Sector {sec}: {comp:.2f}")

    # 4) Condición de parada: todos por debajo del umbral
    if all(comp <= threshold for comp in sector_comp.values()):
        print("\n✅ Todos los sectores están por debajo del umbral. Proceso terminado.")
        break
    else:
        print("⚠️  Aún hay sectores que superan el umbral. Añadiendo otro sector...\n")
        i += 1

# Asignación final
df_cells['Final_Sector_MultiRun'] = df_cells['Cell_Name'].map(assignment)
print(f"\nProceso completo. Total de sectores finales: {len(sector_comp)}")
